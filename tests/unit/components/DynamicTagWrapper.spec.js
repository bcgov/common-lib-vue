...
describe("DynamicTagWrapper.vue", () => {
  ...
  it("receives any passed listeners", async () => {
    ...
    const wrapper = mount({
      ...
      template:
        '<div><DynamicTagWrapper tag="span" @click="handleClick" > 42 </DynamicTagWrapper> </div>',
      components: { DynamicTagWrapper },
    });
  ...
  it("receives any passed attributes", () => {
    const wrapper = mount({
      template:
        '<div><DynamicTagWrapper tag="a" id="test-id" data-cy="cypress-test-id" href="example.com"> 42 </DynamicTagWrapper> </div>',
      components: { DynamicTagWrapper },
    });
