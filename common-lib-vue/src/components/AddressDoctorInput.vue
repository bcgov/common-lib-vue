...
import cypressMixin from '../mixins/cypress-mixin'

const QUERY_REQUEST_DEBOUNCE_TIME = 500
const MIN_LOOKUP_LENGTH = 3

export default {
  name: 'AddressDoctorInput',
  ...
  emits: ['update:modelValue', 'input', 'addressSelected'],
  data() {
    return {
      query: null,
      data: [],
  ...
  methods: {
    ...
    scrollDownToSelectedItem() {
      setTimeout(() => {
        ...
        if (
          this.$refs.resultItemContainer.scrollTop + this.$refs.resultItemContainer.clientHeight <
          selectedItemEl.offsetTop + selectedItemEl.clientHeight
        ) {
          this.$refs.resultItemContainer.scrollTop =
            selectedItemEl.offsetTop -
            this.$refs.resultItemContainer.clientHeight +
            selectedItemEl.clientHeight
    ...
    scrollUpToSelectedItem() {
      setTimeout(() => {
        ...
        if (this.$refs.resultItemContainer.scrollTop > selectedItemEl.offsetTop) {
          ...
          this.$refs.resultItemContainer.scrollTop =
            this.$refs.resultItemContainer.scrollHeight -
            this.$refs.resultItemContainer.clientHeight
        }
      }, 0)
    }
