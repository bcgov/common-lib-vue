<template>
  <div>
    ...
    <div v-if="isLoadingNewCaptcha">
      <LoaderComponent :color="'#AAA'" :size="'20px'" />
    </div>
    <div v-if="!isLoadingNewCaptcha" class="captcha-group-container">
      <div class="captcha-image-container" v-html="captchaSVG" />
    ...
    <div class="captcha-input-container">
      ...
      <input
        id="input-answer"
        v-model="inputAnswer"
        :class="'form-control input-answer field' + (isInputValid === false ? 'border-danger' : '')"
      ...
      />
      <div v-if="isLoadingCaptchaVerification" class="validation-spinner-container">
        <LoaderComponent :color="'#AAA'" :size="'20px'" />
      </div>
    </div>
      ...
      if (!this.isLoadingAudio) {
        ...
        axios
          ...
          .then((response) => {
            ...
            setTimeout(() => {
              //The following code makes audio testable in Jest/Vitest.
...
.captcha-button {
  ...
  color: white;
  padding: 5px 10px;
  /* changed from the default 12px 32px */
  text-align: center;
  text-decoration: none;
  font-size: 10px;
  /* changed from the default 18px */
